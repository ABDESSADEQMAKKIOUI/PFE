...
if (strlen($_SESSION['name']) == 0) {
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO certificat (date_c, formation, nom) VALUES (?, ?, ?)");

    if (!$stmt) {
      die("Error in preparing statement: " . $conn->error);
    }

    $stmt->bind_param("sss", $date_p, $formation_p, $first_name);

    // Execute SQL statement
    if ($stmt->execute()) {
      echo "Data successfully inserted into the database.";
      header('location:ajouter_certificat.php');
    } else {
