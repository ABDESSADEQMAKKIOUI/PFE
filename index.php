...
if (isset($_POST['submit'])) {
    ...
    if ($ret && mysqli_num_rows($ret) > 0) {
        ...
        if ($ret && mysqli_num_rows($ret) > 0) {
            ...
            $_SESSION['cid'] = $row['id'];
            $_SESSION['name'] = $row['username'];
            header('location:secretairy/profil_admin.php');
        } else {
         ...
         if ($result && mysqli_num_rows($result) > 0) {
           ...
           header('location:student/formation.php');
         } else {
            $sql = "SELECT * FROM formateur WHERE username='$username' AND password='$password'";
    $result = mysqli_query($conn, $sql);
    ...
    if ($result && mysqli_num_rows($result) > 0) {
      ...
      header('location:teacher/formation.php');
        // User exists, redirect to the home page or do something else
        exit();
    } else {
        // User doesn't exist or the password is incorrect
